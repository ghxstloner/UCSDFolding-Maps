...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numQuakes, int minimumRepeated) {
		if (minimumRepeated <= 1) {
		 ...
		 List<Float> magnitudes = new ArrayList<Float>(filteredByMinRepetition.keySet());
		 magnitudes.sort(null);
		 int count = 0;
		 
		 for(int i = magnitudes.size()-1; i>= 0 && count < numQuakes; i--) {
	...
	private Map<Float, List<EarthquakeMarker>> filterByRepeatedMagnitude(Map<Float, List<EarthquakeMarker>> input, int minimumRepeated){
		if (minimumRepeated < 1) {
			return new HashMap<>();
